...
if doProductionRuns
    ProductionReps = 15000;
    advanced.MinGridPerStdev = 120;
else
    ProductionReps = 100;
    advanced.MinGridPerStdev = 40;
end
...
tic
if ~exist('fignum','var'), fignum = 20; end;
[basic, advanced] = SetHipArthro_SECTION5();
advanced.dirstring = 'hiparthro';
...
% MF 27/03: for Sinus, HipArthro
numinsimFreqDeltaVec=280;  % can change the number of points in freqdeltavec. The 11/3 in the next line is to get \pm 11/3 standard errors in width for vector's range
advanced.simFreqDeltaVec = (basic.sigma/sqrt(basic.t0)) * (15/3) * (-ceil(numinsimFreqDeltaVec/2):ceil(numinsimFreqDeltaVec/2)) / ceil(numinsimFreqDeltaVec/2); % MF 27/03 changed to 15/03 because otherwise the range was too short (did not extend to the optimal `do nothing' values of mu0)

graphicsuffix = '';
PathReps = 10;      % number of replications for generating sample paths for demonstration
[fignum, mat] = DoSectionFivePlotsHipArthro(fignum, basic,advanced,PathReps,ProductionReps,graphicsuffix);
fignum = DelaySimSurvival( fignum, basic, advanced, mat ) ;
minutestocompute = toc/60
save('HipArthro.mat') ;
...
[basic, advanced] = SetSinus_SECTION5();
advanced.dirstring = 'sinus';
...
%TO THE COURAGEOUS: Try the following instead
numstd=5;
numinsimFreqDeltaVec = ceil(1 + numstd * (basic.sigma/sqrt(basic.t0))/advanced.dmu);
advanced.simFreqDeltaVec = (-numinsimFreqDeltaVec:numinsimFreqDeltaVec) * advanced.dmu; % SEC: to avoid some interpolation issues with mismatch between dmu grid and simFreqDelta grid
...
graphicsuffix = '';
PathReps = 10;      % number of replications for generating sample paths for demonstration
[fignum, mat] = DoSectionFivePlotsSinus(fignum, basic,advanced,PathReps,ProductionReps,graphicsuffix);
fignum = DelaySimSurvival( fignum, basic, advanced, mat ) ;
minutestocompute = toc/60
%save('Sinus.mat') ; 
save Sinus

warning('VALIDATE GRAPHS. IF OK, COPY AND CHECK IN EPS AND PDF FILES IF THEY ARE TO BE PUT INTO PAPER');
