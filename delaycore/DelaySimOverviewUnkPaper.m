function [ figout, matout ] = DelaySimOverviewUnkPaper( fignum, basic, advanced, mat )
...
%    else
        ...
        if advanced.UnkVariance  % if unknonwn variance, then generate noise with draws from unknown variance
            stdevfactor=sqrt(gamrnd(advanced.UnkVarianceShape,1/advanced.UnkVarianceShape,1,NREPS));
            normalnoise = normalnoise .* (ones(basic.TMax,1)*stdevfactor );
        end
        ...
        for i=1:length(advanced.simFreqDeltaVec)
            ...
            [fignum, simOut] = DelaySimComputerUnkPaper(fignum, basic,advanced,mat,delta,delta+bayesmeandelta,normalnoise);  % do simulation analysis for bayesian predictive distribution
            if i==1
        ...
        for i=1:length(advanced.simFreqDeltaVec)
            ...
            [fignum, tmp.simOut(i)] = DelaySimComputerUnkPaper(fignum, basic,advanced,mat,delta,meanvec,normalnoise);
        end
        if advanced.keepAllOutput
