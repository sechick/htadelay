function [figout, mat] = DoSectionFivePlotsHip(fignum,basic,advanced,PathReps,ProductionReps,graphicsuffix)
...
if abs( mat.Threshpoint(3) - mat.Threshpoint(1)) > 1
    scatter (mat.bestsvec(mat.Threshpoint(3):1:mat.Threshpoint(1)) + basic.t0, mat.muvec(mat.Threshpoint(3):1:mat.Threshpoint(1)),'k');
    hold on ; 
end 
...
text(  basic.t0 + 0.5,  plot_lower_tmp + 710, 'n_0', 'Fontsize', tmpfontsize) ;
text(  basic.t0 + basic.TMax - 25,  plot_lower_tmp +  710  , 'n_0 + T_{max}', 'Fontsize', tmpfontsize) ;
text(  basic.t0 + basic.tau + 2.5,  plot_lower_tmp +  710 , 'n_0 + \tau', 'Fontsize', tmpfontsize) ;
...
dirname = 'Figure' ;
UtilSaveFigEpsPdf(fignum,dirname,strcat('pathsHip',graphicsuffix),'-r600');
%savefig paths ; 
...
UtilStdizeFigure(fignum,advanced);
dirname = 'Figure' ;
UtilSaveFigEpsPdf(fignum,dirname, strcat('diffsHip',graphicsuffix),'-r600');
...
legend( 'Optimal Bayes Sequential',  'Fixed (= trial sample size)', 'Optimal Bayes One Stage', 'Location','south' ) ;
hold on ; 
lohivec = [ 0.9, 1 ] ;
UtilPlotABCD( basic, advanced, mat, ishoriz, ist0, lohivec ); 
UtilStdizeFigure(fignum,advanced);
UtilSaveFigEpsPdf(fignum,dirname, strcat('pr_select_bestHip',graphicsuffix),'-r600');
...
hold on ; 
UtilStdizeFigure(fignum,advanced);
UtilSaveFigEpsPdf(fignum,dirname, strcat('E_num_seenHip',graphicsuffix),'-r600');
...
UtilPlotABCD( basic, advanced, mat, ishoriz, ist0, lohivec ); 
UtilStdizeFigure(fignum,advanced);
UtilSaveFigEpsPdf(fignum,dirname, strcat('pr_reversalHip',graphicsuffix),'-r600');
