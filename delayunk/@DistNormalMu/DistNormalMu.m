classdef DistNormalMu < handle
% DistNormalMu:
% Object to represent random variables which have normal distribution with
% unknown mean Mu and known standard deviation Sigma.
% 
% public properties:
...
%           mu0, t0, and Sigma
%       so the unknown mean W ~ Normal(mu0, Sigma^2/t0), and samples have
%       distribution X ~ Normal(W, Sigma^2)
%
...
%   EVariance: (known) sampling variance
%
% public methods: (beyond get/set)
%   Constructor: DistNormalMu(mu0, t0, sig, defaultpar)
%       
...
%
    ...
    methods
        function obj = DistNormalMu(mu0, t0, sig, defaultpar) 
           %Hyperparameter for Normal(W,sig^2) with unknown mean and known
           %standard deviation is taken to be the conjugate
           %Normal(mu0,sigma^2/t0) distribution for the unknown mean W.
           % Default mu0 is 0. Default t0 is 4 (must be greater than 0).
           % Thus, (mu0, t0) is the hyperparameter.
           % defaultpar is set to be a realized value of the unknown mean,
           ...
           % (can be modified with SampleThetaBayes or SampleThetaFreq).
            ...
            if nargin<3
                obj.Sigma = 1;      % default sampling variance is 1
            else
                if sig < 0
                    warning('standard deviation must be at least 0, reset to 1');
                    sig = 1;
                end
                obj.Sigma = sig;
            end
            if nargin<4
                ...
                obj.Thetavec = defaultpar; % unless it is specified
            end
            obj.Samplecdfmat = [];
        ...
        function EVariance = get.EVariance(obj)
            EVariance = obj.Sigma^2;
            % *TO ADAPT*: Implement a function which returns the mean value
            % of the sampling distribution.
        end

        function obj = set.EVariance(obj,~)
            fprintf('%s%f\n','EVariance is: ',obj.Sigma^2)
            ...
            if m*n == 1
                ...
                for i=1:m
                    for j=1:n
                        ...
                        fprintf(1,'Hyperparameters:\n  mu0=%f, sigma=%f, t0=%f\n',...
                            obj(i,j).mu0,obj(i,j).Sigma,obj(i,j).t0);
                        obj(i,j).Thetavec
                    end
                end
            end
    ...
    methods (Access = 'private') % Access by class members only
       function sampledthetas = SampleThetaWorker(obj,m,u)
           ...
           obj.Thetacdfvec = u;
           sampledthetas = norminv(obj.Thetacdfvec,obj.mu0,mystd); % *ADAPT*
        ...
        function samps = SampleWorker(obj,i,n,u)
           ...
           samps = norminv(u(:),obj.Thetavec(i),obj.Sigma); % *ADAPT*
